...
use crate::state::{bank::Bank, local_app::{FocusOn, LocalApp}};
...
impl LocalApp {
    ...
    fn create(&mut self, _ui: &mut egui::Ui, frame: &mut eframe::Frame) {
        if self.is_ok() {
            ...
            self.bank.replace(Bank::default());
    ...
    pub fn render_create_account(&mut self, ui: &mut egui::Ui, frame: &mut eframe::Frame) {
        ...
        .show(ui.ctx(), |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                let res = TextEdit::singleline(&mut self.username).text_color(if is_ok {
                    Color32::GREEN
                } else {
                    Color32::LIGHT_RED
                }).ui(ui);
            ...
            if ui.add_sized(Vec2::new(100.0, 20.0), egui::Button::new("Create")).clicked() {
                self.create(ui, frame);
            }
        });
    }
